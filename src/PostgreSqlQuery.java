...
public class PostgreSqlQuery {
	...
	private static void purchaseAbook() {
		...
		try {
			...
			ResultSet resultSet = statement.executeQuery(
					"Insert into Orders (Tracking_number, Username, Billing_Address, Credit_card, Shipping_Address) values ('SOMETRACKING', 'default', 'Somewhere', 123, 'Elsewhere') RETURNING order_number");
			resultSet.next();
			int order_number = resultSet.getInt(1);
			// decrement the book stock by 1
			resultSet = statement
					.executeQuery("UPDATE Book SET stock = stock - 1 WHERE Isbn =" + isbn + "RETURNING STOCK");
			resultSet.next();
			int stock = resultSet.getInt(1);
			// add the book into the order with the isbn and order number
			statement.executeUpdate(
					"Insert into Order_Item (Order_number, ISBN) values (" + order_number + ", " + isbn + ")");
	...
	private static void deleteBOok() {
		System.out.println("ISBN of the book to be removed");
		String isbn = sc.nextLine(); // Read user input
